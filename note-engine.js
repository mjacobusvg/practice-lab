...
CARRY THE PRIOR DIAGNOSES VERBATIM, INCLUDING SEVERITY/COURSE SPECIFIERS: reproduce each prior diagnosis's ICD-10 code and its exact specifier (e.g. "recurrent, moderate") as the prior assessment stated it. Do NOT re-derive or change a severity specifier (mild / moderate / severe) unless TODAY'S documented data clearly warrants the change; if you do change one, you MUST raise it in the flags with the basis. The diagnosis list — codes, order, and specifiers — is fixed by the prior assessment and today's clinical data ONLY, and must be IDENTICAL regardless of the requested assessment length. The length setting changes how much formulation prose you write; it NEVER changes the diagnoses or their specifiers. If the prior assessment gave no specifier and today's data does not establish one, omit the specifier rather than inventing a severity.
