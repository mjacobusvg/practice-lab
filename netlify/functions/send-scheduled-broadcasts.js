...
exports.handler = async function (event) {
  ...
  try {
    ...
    const dueRes = await fetch(URL + '/rest/v1/scheduled_broadcasts?status=eq.scheduled&scheduled_at=lte.' + encodeURIComponent(nowIso) + '&order=scheduled_at.asc&select=id,subject,markdown,audience&limit=20', { headers: auth });
    ...
    for (const b of due) {
      ...
      try {
        const sendRes = await fetch(base + '/.netlify/functions/broadcast-send', {
          ...
          body: JSON.stringify({ internal_secret: SECRET, subject: b.subject, markdown: b.markdown, audience: b.audience })
        });
