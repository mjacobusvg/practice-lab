...
// This file registers the Ask the Archive pipeline function with Inngest.
      ...
      async function runSearch(variants) {
        ...
        const searchResults = await Promise.all(embeddings.map(function(emb) {
          return fetch(`${supabaseUrl}/rest/v1/rpc/match_posts`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'apikey': supabaseKey, 'Authorization': `Bearer ${supabaseKey}` },
            body: JSON.stringify({ query_embedding: emb, match_threshold: conversationHistory.length > 0 ? 0.50 : MATCH_THRESHOLD, match_count: conversationHistory.length > 0 ? 8 : MATCH_COUNT })
        ...
        return Object.values(map).sort(function(a, b) { return (b.similarity||0)-(a.similarity||0); }).slice(0, conversationHistory.length > 0 ? 8 : MATCH_COUNT);
      }
...
For answered questions:
{
  ...
  "source_descriptions": [
    ...
    { "index": 2, "description": "max 10 words describing what clinical issue this source covers" },
    { "index": 3, "description": "...and so on for EVERY source up to the last index" }
  ],
  "template_sources": [
...
CRITICAL for source_descriptions: You MUST include one entry for every single source index from 1 to N where N is the total number of sources provided. Do not skip any index. Do not omit any source. Every source gets a description — no exceptions.
...
Return ONLY the JSON object. Nothing before or after it.`;
    ...
    console.log('Sources total:', enrichedChunks.length, '| Descriptions returned:', (parsed.source_descriptions || []).length, '| Token usage:', claudeData.usage ? JSON.stringify(claudeData.usage) : 'unknown');
    let sourceDescMap = {};
    try {
...
// Serve handler export — Inngest calls this endpoint for registration and invocation
export const handler = serve({
  client: inngest,
  functions: [askArchivePipeline],
