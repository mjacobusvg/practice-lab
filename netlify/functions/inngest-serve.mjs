...
function hybridMerge(vectorResults, ftsResults, queryKeywords) {
  ...
  const results = Object.keys(scores).map(function(id) {
    ...
    // Member posts and comments are useful context but should not outrank admin content
    // on the same topic. This prevents incorrect member comments from being weighted
    // equally with carefully written admin posts.
    const authorLower = (r.author || '').toLowerCase();
    const isMichaelPost = authorLower.includes('michael van gelder') || authorLower.includes('michael');
    ...
    // Top-level posts (id starts with post_ but not comment_) are the primary content.
