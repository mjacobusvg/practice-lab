...
async function ccCallAnthropic(systemPrompt, userMessage, anthropicKey, maxTokens) {
  ...
  let text = '';
  if (data.content) data.content.forEach(function(c) { if (c.type === 'text') text += c.text; });
  return text;
}
...
const chartCoderPipeline = inngest.createFunction(
  ...
  async function({ event }) {
    ...
    async function saveResult(status, result) {
      await fetch(`${supabaseUrl}/rest/v1/tool_jobs?job_id=eq.${job_id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'apikey': supabaseKey,
    ...
    try {
      ...
      // Pass A: Audit (parallel with MDM for speed)
      const auditP = ccCallAnthropic(CC_AUDIT_PROMPT, 'Chart note to audit:\n\n' + noteText, anthropicKey, 2000);
      const mdmP = ccCallAnthropic(CC_MDM_EVAL_PROMPT, userMsg, anthropicKey, 2000);
      const [auditRaw, mdmRaw] = await Promise.all([auditP, mdmP]);
      const audit = ccParseJSON(auditRaw);
      const mdm = ccParseJSON(mdmRaw);

      // Pass B: Review (depends on MDM)
      ...
      try { await saveResult('error', { error: err.message }); } catch(e) {}
    }
  }
);
