// netlify/functions/ingest-cms-doc-upload-background.js
...
exports.handler = async function(event, context) {
  ...
  (async () => {
    try {
      ...
      console.log(`[ingest-cms-doc-upload-background] Extracted ${extractedText.length} chars`);

      const chunks = chunkText(extractedText, 7000);
      console.log(`[ingest-cms-doc-upload-background] ${chunks.length} chunk(s)`);
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        console.log(`[ingest-cms-doc-upload-background] Upserted chunk ${i + 1}/${chunks.length} (id: ${chunkId})`);
        await sleep(500);
      }
...
async function getEmbedding(text, apiKey) {
  ...
  if (!resp.ok) { const err = await resp.text(); throw new Error(`OpenAI embedding failed: ${err.substring(0, 200)}`); }
...
async function upsertToSupabase(url, key, record) {
  ...
  if (!resp.ok) { const err = await resp.text(); throw new Error(`Supabase upsert failed: ${err.substring(0, 200)}`); }
}
