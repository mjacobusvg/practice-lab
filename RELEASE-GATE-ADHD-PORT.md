...
| D.7 | After a mid-visit check, "Add these N" adds the questions THAT CHECK raised, not the whole baseline list again | **PASS 26 Sep.** "Added 3 questions to the bottom of CHECK / ASK TODAY. Nothing already in your note was changed." Exactly the three ASK NEXT items; the baseline eight were not re-added. |
| D.8 | Framework auto-collapse: the mid-visit card collapses the Framework behind it and speaks in fragments | **PASS 26 Sep.** Card rendered top-right with the Framework shut behind it. Three bullets under WHAT CHANGED, three under ASK NEXT, all telegraphic ("Mother's 'straight-A' account attributed by Dana to memory confusion with her sister." -- 12 words, no explanatory clause). WHY IT MATTERS read "discrepancy now runs both directions": it registered a retracted collateral account without flipping to believe the patient. Note the card only fires with ~400+ characters of new VISIT material; adding a record routes to the full rebuild instead, correctly, since `tbpAdhdDelta` reads `tbpAdhdVisitMaterial()` and cannot see a new source document. |
| D.9 | Framework questions append to the working note even when the note already has content | **PASS 26 Sep**, with D.7. The note held eight questions and a typed paragraph; the append disturbed none of it. |
| D.10 | Headings and context prose from the post-visit pass do NOT land in CHECK / ASK TODAY as questions | **NOT EXERCISED.** The 26 Sep delta produced no grouped headings, so the case this guards against never arose. Left open rather than claimed. |
...
| E (pharmacology rule changed) | The assessment stated "atomoxetine typically requires four to eight weeks at therapeutic dosing" as fact. **Correction to an earlier entry: this broke no rule.** The bar on verifiable prescribing facts lives in `reasonSystem()` (Discern) only; `ASSESS_SYS` in `note-engine-practice.js` has no such rule. **Michael's ruling, 26 Sep: the facts stay.** Stripping pharmacology to avoid being wrong produces a conclusion with its reasoning removed, which is not safer, and is how the tool becomes a yes-man. Rewritten in ambient-143-sub as fact vs verdict: state onset, duration, half-life, labeled maxima, metabolic pathway and interactions; withhold only the prescribing decision -- no "you can go to X mg for this patient", no contraindicated/safe verdict. A real interaction prescribed daily with monitoring is not a contraindication. The DO NOT RESCUE rule survives, re-grounded: the fault is reaching for pharmacology to justify a conclusion you already wanted, not stating it. |
