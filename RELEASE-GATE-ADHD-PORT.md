...
| MSE (C.4) | `Affect is congruent with reported mood.` printed alongside `Mood is not reported.` **Scope confirmed by test:** document a mood and the line is correct, so this is a null-mood coupling bug, not an MSE problem. Every other MSE default is self-contained; only this one references another field. Fix is one conditional: with no documented mood, describe affect on its own terms (`Affect is full range and appropriate to content`). Queued for the first post-port build, since the MSE renders in every draft and changing it invalidates C.1 and C.4. |
